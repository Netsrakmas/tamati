...
export function makeGround(screenW: number, screenH: number): Ground {
  return { horizonY: Math.round(screenH * 0.52), screenW, screenH };
}
...
export function worldToScreen(
  ...
  return {
    ...
    x: g.screenW / 2 + p.x * (g.screenW * 0.34) * (0.45 + 0.55 * y),
    y: g.horizonY + y * (g.screenH * 0.36),
    scale: s * Math.min(g.screenW / 340, g.screenH / 470, 1.45),
  };
}
...
export function screenToWorldX(
  ...
  const y = clamp01(atY);
  const halfSpan = g.screenW * 0.34 * (0.45 + 0.55 * y);
