    ...
    try {
      ...
      if (url.pathname.startsWith("/upgrade/")) {
        ...
        if (url.pathname === "/upgrade/sw.js") {
          ...
          res.end(
            template
              .replace("__RELEASE__", `upgrade-${current}`)
              .replace("__ASSETS__", JSON.stringify(["./", `./${current}.js`])),
          );
        } else if (url.pathname === "/upgrade/") {
          res.setHeader("content-type", "text/html");
          res.end(
            `<html><body><script src="./${current}.js"></script></body></html>`,
          );
        } else if (url.pathname === `/upgrade/${current}.js`) {
          res.setHeader("content-type", "text/javascript");
          res.end(
            `document.body.dataset.release='${current}';document.body.append('${current}');navigator.serviceWorker.register('./sw.js')`,
          );
        } else res.writeHead(404).end("Previous release removed from server");
        return;
      }
      const route = url.pathname.replace(/^\/tamati\//, "/");
      let p = join(ROOT, route === "/" ? "index.html" : route);
...
async function waitReady(page) {
  await page.waitForFunction(() => window.__tamati?.ready === true, {
    timeout: 15000,
  });
}
...
async function main() {
  ...
  // A nightly CI run must not mistake intentional sleep for a broken greeting.
  await ctx.addInitScript(() => {
    const NativeDate = Date;
    window.Date = class extends NativeDate {
      constructor(...args) {
        super(...(args.length ? args : [2026, 8, 17, 12, 0, 0]));
      }
      static now() {
        return new NativeDate(2026, 8, 17, 12, 0, 0).getTime();
      }
    };
  });
  ...
  // ---------------------------------------------------------------- A4 + A2
  {
    ...
    const nav = await page.evaluate(() => {
      ...
      const mark = performance.getEntriesByName(
        "tamati-first-greeting-frame",
      )[0];
      return {
        domContentLoaded: e?.domContentLoadedEventEnd ?? 0,
        firstFrame: mark?.startTime ?? 0,
      };
    });
    cold < 1500
      ? pass(
          "A4 cold start → first greeting frame",
          `${cold}ms (budget 1500ms)`,
        )
      : fail(
          "A4 cold start → first greeting frame",
    ...
    canvasOk
      ? pass("canvas present and sized")
      : fail("canvas present and sized");
    await page.close();
    if (errors.length)
      fail("A2 console clean on boot", errors.slice(0, 3).join(" | "));
  ...
  for (const [name, qs] of tiers) {
    ...
    await page.goto(`${BASE}/${qs}${qs ? "&" : "?"}debug`, {
    ...
    await page.close();
    if (errors.length)
      fail(`A2 console clean (${name})`, errors.slice(0, 2).join(" | "));
  }
  pass("A2 console clean across all five tiers");

  // Settings remain usable while first-load artwork is still downloading.
  {
    ...
    const { errors } = watch(page);
    await page.route(/\/assets\/room-.*\.png$/, async (route) => {
    ...
    await page
      ...
      .click();
    await page.getByLabel("Your companion’s name").fill("Pixel");
    await page.getByLabel("Room light", { exact: true }).selectOption("night");
    await page
      .getByRole("button", { name: "Save settings", exact: true })
      .click();
    await waitReady(page);
    const savedDuringBoot = await page.evaluate(() => {
      const save = JSON.parse(localStorage.getItem("tamati.save"));
      return (
        save.name === "Pixel" &&
        save.light === "night" &&
        document.querySelector(".room-frame").dataset.time === "night"
      );
    });
    savedDuringBoot && !errors.length
      ? pass("settings saved during slow art loading apply on first frame")
      : fail("settings during slow art loading", errors.join(" | "));
    await slow.close();
  }
  ...
  // ------------------------------------------------------------- wandering
  {
    ...
    await waitReady(page);
    await page.waitForFunction(() => window.__tamati.greetingActive === false, {
      timeout: 8000,
    });
    ...
    moved > 0.12
      ? pass(
          "pet wanders the ground plane",
          `moved ${moved.toFixed(2)} world units`,
        )
      : fail("pet wanders the ground plane", `only moved ${moved.toFixed(2)}`);
    sawWalking
      ? pass("walking state is reported")
      : fail("walking state is reported");
    ...
    await page.close();
    if (errors.length)
      fail("A2 console clean while wandering", errors.slice(0, 2).join(" | "));
  ...
  // ------------------------------------------------------ grab / drag / shake
  {
    ...
    const before = await page.evaluate(() =>
      window.__tamati.rig.find((p) => p.name === "head"),
    );
    ...
    grabbing
      ? pass("grab registers on the pet")
      : fail("grab registers on the pet");
    ...
    shakeResult.samples > 0
      ...
      : fail(
          "shake input path reaches the detector",
          "no pointer samples registered at all",
        );
    ...
    if (sampleHz < RELIABLE_HZ) {
      ...
      pass(
        "shake detected (4Hz gesture)",
        `${shakeResult.reversals} reversals at ${sampleHz.toFixed(0)}Hz`,
      );
    } else {
      fail(
        "shake detected (4Hz gesture)",
    ...
    const finite = await page.evaluate(() =>
      window.__tamati.rig.every(
        (p) => Number.isFinite(p.x) && Number.isFinite(p.y),
      ),
    );
    finite
      ? pass("rig finite after drag/shake/drop")
      : fail("rig finite after drag/shake/drop");
    ...
    cpuMean < 4
      ...
      : fail(
          "A1 (partial) sim+draw CPU cost",
          `mean ${cpuMean.toFixed(2)}ms is too much of 16ms`,
        );
    results.push({
      ok: true,
      n: "Desktop frame timing (not an iPhone performance certification)",
      d: `${fps.toFixed(1)} fps mean / worst ${worst.toFixed(1)}ms / ${over16} frames >16.7ms — native iOS still needs device testing`,
      info: true,
    });
    ...
    errors.length
      ? fail(
          "A2 console clean during interaction",
  ...
  // ---------------------------------------------------- finished-product flows
  {
    ...
    for (const [action, label, completed] of [
      ["snack", "Food", "Crumbs everywhere. Absolutely worth it."],
      ["clean", "Wash", "Fresh as a daisy. Slightly more bubbly."],
      ["play", "Play", "No winner. No score. Just a very good ball."],
    ]) {
      await page.getByRole("button", { name: label, exact: true }).click();
      const active = await page.evaluate(() => window.__tamati.care);
      active === action
        ? pass(`${action} starts its own reaction`)
        : fail(`${action} starts its own reaction`);
      await page.waitForTimeout(1000);
      await page.screenshot({ path: join(SHOTS, `product-${action}.png`) });
      await page.waitForFunction(() => window.__tamati.care === null);
      const message = await page.getByRole("status").innerText();
      message === completed
        ? pass(`${action} completes and gives feedback`)
        : fail(`${action} completes`, message);
    }
    await page.waitForFunction(() =>
      document.getElementById("status").textContent.includes("All cosy"),
    );
    pass("visit settles without a further demand");
    await page
      .getByRole("button", { name: "Let your companion rest", exact: true })
      .click();
    await page.waitForFunction(() => window.__tamati.face === "asleep");
    const sleepPos = await page.evaluate(() => window.__tamati.world);
    await page.waitForTimeout(600);
    const still = await page.evaluate(() => window.__tamati.world);
    JSON.stringify(sleepPos) === JSON.stringify(still)
      ? pass("nap stays asleep without wandering")
      : fail("nap stays asleep without wandering");
    await page
      .getByRole("button", { name: "Wake your companion", exact: true })
      .click();
    await page.waitForFunction(() => !window.__tamati.sleeping);
    pass("wake resumes ordinary play");

    await page
      .getByRole("button", { name: "Room settings", exact: true })
      .click();
    await page.getByLabel("Your companion’s name").fill("Sprout");
    await page.getByLabel("Room light", { exact: true }).selectOption("day");
    ...
    const saved = await page.evaluate(() =>
      JSON.parse(localStorage.getItem("tamati.save")),
    );
    saved.name === "Sprout" &&
    saved.light === "day" &&
    saved.reducedMotion &&
    Object.values(saved.care).every(Boolean)
      ? pass("name, settings and completed care survive reload")
      : fail("settings persistence", JSON.stringify(saved));
    await page
      .getByRole("button", { name: "Room settings", exact: true })
      .click();
    await page
    ...
    !(await page.getByRole("dialog").isVisible())
      ? pass("field guide dismisses with Escape")
      : fail("field guide dismisses with Escape");
    const canvas = page.locator("canvas");
    await page.waitForFunction(() => !window.__tamati.greetingActive);
    await canvas.focus();
    await page.keyboard.press("Enter");
    const tapLevel = await page.evaluate(() => window.__tamati.tapLevel);
    tapLevel > 0
      ? pass("keyboard can greet the pet")
      : fail("keyboard can greet the pet");
    ...
    for (const [width, height] of [
      ...
      const fits = await page.evaluate(() => {
        ...
        const buttons = [...document.querySelectorAll("[data-care]")].map((e) =>
      ...
      fits
        ? pass(`responsive layout ${width}×${height}`)
        : fail(`responsive layout ${width}×${height}`);
      await page.screenshot({
        path: join(SHOTS, `product-${width}.png`),
        fullPage: true,
      });
    }
    errors.length
      ? fail("product flows have no browser errors", errors.join(" | "))
      : pass("product flows have no browser errors");
    await page.close();
  }

  // GitHub Pages lives under a subpath, not the origin root.
  {
    const page = await ctx.newPage();
    const { errors } = watch(page);
    await page.goto(`${BASE}/tamati/`);
    await waitReady(page);
    await page.evaluate(() => navigator.serviceWorker.ready);
    errors.length
      ? fail("GitHub Pages subpath boots cleanly", errors.join(" | "))
      : pass("GitHub Pages subpath boots cleanly");
    await page.close();
  }
  ...
  // ---------------------------------------------------------------- A6 offline
  {
    ...
    try {
      ...
      await page
        ...
        .click();
      await page
        .getByLabel("Room light", { exact: true })
        .selectOption("night");
      await page
        .getByRole("button", { name: "Save settings", exact: true })
        .click();
      ...
      await page.screenshot({ path: join(SHOTS, "product-night-offline.png") });
      pass("night artwork and lighting work offline");
    } catch {
      fail(
        "A6 works offline after first load",
        "did not become ready with network off",
      );
    }
    await ctx.setOffline(false);
    await page.close();
    if (errors.length)
      fail("offline console clean", errors.slice(0, 2).join(" | "));
  }

  // A real two-release deployment. Old HTML must still get its old bundle after the
  // server removes it; the waiting update must activate when the old app is closed.
  {
    const upgrade = await browser.newContext();
    const oldPage = await upgrade.newPage();
    await oldPage.goto(`${BASE}/upgrade/`);
    ...
    await oldPage.waitForFunction(
      () => document.body.dataset.release === "one",
    );
    pass("open app keeps its complete previous release during an update");
    await oldPage.close();
    const newPage = await upgrade.newPage();
    await newPage.waitForTimeout(300);
    await newPage.goto(`${BASE}/upgrade/`);
    await newPage.waitForFunction(
      () => document.body.dataset.release === "two",
    );
    pass("new release activates after the previous app closes");
    await upgrade.setOffline(true);
    await newPage.reload();
    await newPage.waitForFunction(
      () => document.body.dataset.release === "two",
    );
    pass("updated release also boots fully offline");
    await upgrade.close();
  }
  ...
  console.log("\n─── Tamati product gauntlet ───");
  for (const r of results)
    console.log(
      `${r.info ? "INFO" : r.ok ? "PASS" : "FAIL"}  ${r.n}${r.d ? `  — ${r.d}` : ""}`,
    );
  const failed = results.filter((r) => !r.ok);
  const checked = results.filter((r) => !r.info);
  console.log(
